...
';
     ...
     *
     * Includes social media links if configured in WordPress options.
     *
     * @return string HTML
     * @since 6.13.14 Added social media links support
     */
    private static function get_email_footer() {
        $unsubscribe_url = home_url('/saved-search/');
        $site_name = get_bloginfo('name');
        $site_url = home_url();

        // Get social media URLs from WordPress options - Added v6.13.14
        ...
        // Build social media links if any are configured
        $social_links = [];
        if ($facebook_url) {
        ...
        if (!empty($social_links)) {
            $html .= '
                            <p style="margin:0 0 15px 0;">' . implode('', $social_links) . '</p>';
        }
        ...
        $html .= '
                            <p style="margin:0 0 12px 0;color:#4a4a4a;font-size:15px;line-height:1.5;">
                                You\'re receiving this email because you created a saved search on <a href="' . esc_url($site_url) . '" style="color:#1e3a5f;font-weight:500;">' . esc_html($site_name) . '</a>
                            </p>
                            <p style="margin:0;font-size:14px;">
                                <a href="' . esc_url($unsubscribe_url) . '" style="color:#4a4a4a;text-decoration:underline;">Manage your saved searches</a>
                                <span style="color:#a3a3a3;margin:0 8px;">|</span>
                                <a href="' . esc_url($site_url) . '" style="color:#4a4a4a;text-decoration:underline;">Visit our website</a>
                            </p>
                        </td>
                    </tr>
                </table>
