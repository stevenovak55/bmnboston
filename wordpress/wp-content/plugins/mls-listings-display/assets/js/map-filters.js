...
const MLD_Filters = {
  ...
  initSearchAndFilters() {
    ...
    $('#bme-filter-baths, #bme-filter-garage-spaces, #bme-filter-parking-total').on(
      'click',
      'button',
  ...
  getModalState(isForCountOrOptions = false) {
    ...
    state.beds = $('#bme-filter-beds button.active:not([data-value="0"])')
  ...
  restoreModalUIToState() {
    ...
    if (modalFilters.beds.length > 0) {
      modalFilters.beds.forEach((bed) =>
        $(`#bme-filter-beds button[data-value="${bed}"]`).addClass('active')
      );
    } else {
