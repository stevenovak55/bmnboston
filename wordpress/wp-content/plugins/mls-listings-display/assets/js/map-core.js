...
const MLD_Core = {
  ...
  createCardHTML(listing, context = 'sidebar') {
    ...
    if (isExclusive) {
      const exclusiveTag = listing.exclusive_tag || listing.ExclusiveTag || 'Exclusive';
      ...
      exclusiveBadgeHTML = `<div class="mld-exclusive-badge">${starIcon} ${exclusiveTag}</div>`;
    }
    ...
    const isCommercial = ['Commercial Sale', 'Commercial Lease', 'Business Opportunity'].includes(propertyTypeRaw);
    let schoolBadgeHTML = '';
    if (!isCommercial && listing.district_grade) {
    ...
    } else if (!isCommercial && listing.BestSchoolGrade) {
