...
class MLD_Email_Template_Engine {
    ...
    public function render($template_name, $data = array()) {
        ...
        $html .= $this->render_component('footer', array(
            'agent' => $this->agent_data,
            'unsubscribe_url' => $data['unsubscribe_url'] ?? home_url('/my-account/saved-searches/'),
        ));
...
';
    ...
    private function component_footer($vars) {
        $theme = $this->theme;
        $agent = $vars['agent'] ?? null;
        $unsubscribe_url = $this->get_tracked_url($vars['unsubscribe_url'] ?? home_url('/my-account/saved-searches/'), 'unsubscribe');
...
';
        ...
        // Agent contact reminder (preserved from original)
        if ($agent) {
...
';
        ...
        // Use unified footer from MLD_Email_Utilities (v6.63.0)
        if (class_exists('MLD_Email_Utilities')) {
            ...
            // Fallback to basic footer if utilities not available
            $site_name = get_bloginfo('name');
            $site_url = home_url();
            $html .= '                            <p style="margin:0;color:' . esc_attr($theme['muted_color']) . ';font-size:14px;">
                                <a href="' . esc_url($site_url) . '" style="color:' . esc_attr($theme['primary_color']) . ';">' . esc_html($site_name) . '</a>
                            </p>
';
        }
