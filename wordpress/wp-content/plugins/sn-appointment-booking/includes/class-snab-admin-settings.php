...
class SNAB_Admin_Settings {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_snab_save_push_settings', array($this, 'ajax_save_push_settings'));
        add_action('wp_ajax_snab_test_push_notification', array($this, 'ajax_test_push_notification'));
    }
    ...
    public function render() {
        ...
        <div class="wrap snab-admin-wrap">
            ...
            <nav class="nav-tab-wrapper snab-settings-tabs">
                ...
                <a href="<?php echo esc_url(add_query_arg('tab', 'push-notifications', remove_query_arg(array('snab_success', 'snab_error')))); ?>"
                   class="nav-tab <?php echo $current_tab === 'push-notifications' ? 'nav-tab-active' : ''; ?>">
                    <span class="dashicons dashicons-smartphone"></span>
                    <?php esc_html_e('Push Notifications', 'sn-appointment-booking'); ?>
                </a>
            </nav>
            ...
            <?php elseif ($current_tab === 'push-notifications'): ?>
                <?php $this->render_push_notifications_tab(); ?>
            <?php else: ?>
    ...
    /**
     * AJAX handler to save push notification settings.
     *
     * @since 1.8.5
     */
    public function ajax_save_push_settings() {
        check_ajax_referer('snab_admin_nonce', 'nonce');
        ...
        // Sanitize and save settings
        $enable_push = isset($_POST['enable_push_notifications']) ? (bool) $_POST['enable_push_notifications'] : false;
        update_option('snab_enable_push_notifications', $enable_push);

        // APNs credentials - only update if provided (don't clear existing if blank)
        $key_id = isset($_POST['apns_key_id']) ? sanitize_text_field($_POST['apns_key_id']) : '';
        $team_id = isset($_POST['apns_team_id']) ? sanitize_text_field($_POST['apns_team_id']) : '';
        $auth_key = isset($_POST['apns_auth_key']) ? sanitize_textarea_field($_POST['apns_auth_key']) : '';

        if (!empty($key_id)) {
            update_option('snab_apns_key_id', $key_id);
        }
        if (!empty($team_id)) {
            update_option('snab_apns_team_id', $team_id);
        }
        if (!empty($auth_key) && $auth_key !== '••••••••') {
            update_option('snab_apns_auth_key', $auth_key);
        }

        wp_send_json_success(array(
            'message' => __('Push notification settings saved successfully.', 'sn-appointment-booking'),
        ));
    }

    /**
     * AJAX handler to test push notification.
     *
     * @since 1.8.5
     */
    public function ajax_test_push_notification() {
        ...
        $devices = $push->get_user_devices(get_current_user_id());

        if (empty($devices)) {
            wp_send_json_error(__('No registered devices found for your account. Please log in to the iOS app first.', 'sn-appointment-booking'));
        }

        // Send test notification
        $title = __('Test Notification', 'sn-appointment-booking');
        $body = __('This is a test notification from SN Appointment Booking.', 'sn-appointment-booking');
        $data = array(
            'type' => 'test',
        );
        ...
        if ($sent > 0) {
            wp_send_json_success(array(
                'message' => sprintf(
                    __('Test notification sent to %d device(s).', 'sn-appointment-booking'),
                    $sent
                ),
                'sent' => $sent,
                'failed' => $failed,
            ...
            wp_send_json_error(sprintf(
                __('Failed to send test notification. Error: %s', 'sn-appointment-booking'),
                implode(', ', $errors)
            ));
        }
    }
     ...
     * @since 1.8.5
     */
    private function render_push_notifications_tab() {
        // Get current settings
        $enable_push = get_option('snab_enable_push_notifications', false);
        $key_id = get_option('snab_apns_key_id', '');
        $team_id = get_option('snab_apns_team_id', '');
        $auth_key = get_option('snab_apns_auth_key', '');

        // Check MLD credentials as fallback indicator
        $mld_key_id = get_option('mld_apns_key_id', '');
        $mld_team_id = get_option('mld_apns_team_id', '');
        $mld_auth_key = get_option('mld_apns_private_key', '');
        $has_mld_credentials = !empty($mld_key_id) && !empty($mld_team_id) && !empty($mld_auth_key);
        $has_snab_credentials = !empty($key_id) && !empty($team_id) && !empty($auth_key);

        // Determine credential status
        if ($has_snab_credentials) {
            $credential_status = 'snab';
            $status_class = 'notice-success';
            $status_text = __('Using SNAB-specific APNs credentials', 'sn-appointment-booking');
        } elseif ($has_mld_credentials) {
            $credential_status = 'mld';
            $status_class = 'notice-warning';
            $status_text = __('Using MLD plugin credentials (fallback)', 'sn-appointment-booking');
        } else {
            $credential_status = 'none';
            $status_class = 'notice-error';
            $status_text = __('No APNs credentials configured - push notifications will not work', 'sn-appointment-booking');
        }
        ?>

        <div class="snab-settings-section">
            <h2><?php esc_html_e('Push Notification Settings', 'sn-appointment-booking'); ?></h2>
            <p class="description">
                <?php esc_html_e('Configure push notifications for appointment reminders sent to iOS devices.', 'sn-appointment-booking'); ?>
            </p>

            <!-- Credential Status -->
            <div class="notice <?php echo esc_attr($status_class); ?> inline" style="margin: 15px 0;">
                <p>
                    <strong><?php esc_html_e('Credential Status:', 'sn-appointment-booking'); ?></strong>
                    <?php echo esc_html($status_text); ?>
                </p>
            </div>

            <form id="snab-push-settings-form" class="snab-settings-form">
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <?php esc_html_e('Enable Push Notifications', 'sn-appointment-booking'); ?>
                        </th>
                        <td>
                            <label>
                                <input type="checkbox" name="enable_push_notifications" value="1" <?php checked($enable_push, true); ?>>
                                <?php esc_html_e('Send push notification reminders for appointments', 'sn-appointment-booking'); ?>
                            </label>
                            <p class="description">
                                <?php esc_html_e('When enabled, users will receive push notifications 24 hours and 1 hour before their appointments.', 'sn-appointment-booking'); ?>
                            </p>
                        </td>
                    </tr>
                </table>

                <h3><?php esc_html_e('APNs Credentials (Optional)', 'sn-appointment-booking'); ?></h3>
                <p class="description">
                    <?php esc_html_e('Configure SNAB-specific APNs credentials, or leave blank to use MLD plugin credentials.', 'sn-appointment-booking'); ?>
                </p>

                <?php if ($has_mld_credentials && !$has_snab_credentials): ?>
                    <div class="notice notice-info inline" style="margin: 10px 0;">
                        <p>
                            <span class="dashicons dashicons-info"></span>
                            <?php esc_html_e('You can leave these fields blank to continue using MLD plugin credentials for push notifications.', 'sn-appointment-booking'); ?>
                        </p>
                    </div>
                <?php endif; ?>
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="snab-apns-key-id"><?php esc_html_e('APNs Key ID', 'sn-appointment-booking'); ?></label>
                        </th>
                        <td>
                            <input type="text" id="snab-apns-key-id" name="apns_key_id"
                                   value="<?php echo esc_attr($key_id); ?>"
                                   class="regular-text" placeholder="<?php echo $has_mld_credentials ? esc_attr__('Using MLD: ', 'sn-appointment-booking') . substr($mld_key_id, 0, 4) . '****' : ''; ?>">
                            <p class="description"><?php esc_html_e('10-character Key ID from Apple Developer Portal.', 'sn-appointment-booking'); ?></p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">
                            <label for="snab-apns-team-id"><?php esc_html_e('Apple Team ID', 'sn-appointment-booking'); ?></label>
                        </th>
                        <td>
                            <input type="text" id="snab-apns-team-id" name="apns_team_id"
                                   value="<?php echo esc_attr($team_id); ?>"
                                   class="regular-text" placeholder="<?php echo $has_mld_credentials ? esc_attr__('Using MLD: ', 'sn-appointment-booking') . substr($mld_team_id, 0, 4) . '****' : ''; ?>">
                            <p class="description"><?php esc_html_e('10-character Team ID from Apple Developer Portal.', 'sn-appointment-booking'); ?></p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row">
                            <label for="snab-apns-auth-key"><?php esc_html_e('APNs Auth Key (.p8)', 'sn-appointment-booking'); ?></label>
                        </th>
                        <td>
                            <textarea id="snab-apns-auth-key" name="apns_auth_key"
                                      rows="5" class="large-text code"
                                      placeholder="-----BEGIN PRIVATE KEY-----&#10;...&#10;-----END PRIVATE KEY-----"><?php echo !empty($auth_key) ? '••••••••' : ''; ?></textarea>
                            <p class="description">
                                <?php esc_html_e('Paste the contents of your .p8 key file. Include the BEGIN/END PRIVATE KEY lines.', 'sn-appointment-booking'); ?>
                                <?php if ($has_mld_credentials && empty($auth_key)): ?>
                                    <br><em><?php esc_html_e('Currently using MLD credentials.', 'sn-appointment-booking'); ?></em>
                                <?php endif; ?>
                            </p>
                        </td>
                    </tr>
                ...
                <p class="submit">
                    <button type="submit" class="button button-primary" id="snab-save-push-settings">
                        <?php esc_html_e('Save Settings', 'sn-appointment-booking'); ?>
                    </button>
                    <button type="button" class="button button-secondary" id="snab-test-push" style="margin-left: 10px;">
                        <?php esc_html_e('Send Test Notification', 'sn-appointment-booking'); ?>
                    </button>
                    <span class="spinner"></span>
                </p>
            </form>
        ...
        <!-- Setup Instructions -->
        <div class="snab-settings-section">
            <h2><?php esc_html_e('APNs Setup Instructions', 'sn-appointment-booking'); ?></h2>
            <p class="description">
                <?php esc_html_e('To configure your own APNs credentials:', 'sn-appointment-booking'); ?>
            </p>
            <ol>
                <li><?php esc_html_e('Log in to the Apple Developer Portal.', 'sn-appointment-booking'); ?></li>
                <li><?php esc_html_e('Go to Certificates, Identifiers & Profiles → Keys.', 'sn-appointment-booking'); ?></li>
                <li><?php esc_html_e('Create a new key with Apple Push Notifications service (APNs) enabled.', 'sn-appointment-booking'); ?></li>
                <li><?php esc_html_e('Download the .p8 file and note the Key ID.', 'sn-appointment-booking'); ?></li>
                <li><?php esc_html_e('Find your Team ID on the Membership page.', 'sn-appointment-booking'); ?></li>
                <li><?php esc_html_e('Enter all three values above and save.', 'sn-appointment-booking'); ?></li>
            </ol>
            <p>
                <strong><?php esc_html_e('Bundle ID:', 'sn-appointment-booking'); ?></strong>
                <code>com.bmnboston.app</code>
            </p>
        </div>
        ...
        jQuery(document).ready(function($) {
            ...
            // Save push settings
            $('#snab-push-settings-form').on('submit', function(e) {
                e.preventDefault();

                var $form = $(this);
                var $button = $('#snab-save-push-settings');
                var $spinner = $button.siblings('.spinner');

                // Get auth key value - don't send placeholder
                var authKey = $('#snab-apns-auth-key').val();
                if (authKey === '••••••••') {
                    authKey = '';
                }
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'snab_save_push_settings',
                        nonce: nonce,
                        enable_push_notifications: $('input[name="enable_push_notifications"]').is(':checked') ? 1 : 0,
                        apns_key_id: $('#snab-apns-key-id').val(),
                        apns_team_id: $('#snab-apns-team-id').val(),
            ...
            $('#snab-test-push').on('click', function() {
                var $button = $(this);
                var $spinner = $button.siblings('.spinner');

                if (!confirm('<?php echo esc_js(__('Send a test notification to your registered devices?', 'sn-appointment-booking')); ?>')) {
                    return;
                }
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    data: {
                        action: 'snab_test_push_notification',
                        nonce: nonce
                    },
                    success: function(response) {
                        if (response.success) {
