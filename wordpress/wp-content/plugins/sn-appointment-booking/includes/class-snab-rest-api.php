...
// Load shared JWT handler from MLD plugin
require_once WP_PLUGIN_DIR . '/mls-listings-display/includes/class-mld-jwt-handler.php';
...
class SNAB_REST_API {
    ...
    // ============================================================
    // JWT Authentication: Uses shared MLD_JWT_Handler class
    // See: mls-listings-display/includes/class-mld-jwt-handler.php
    // Methods: check_auth(), check_optional_auth(), send_no_cache_headers()
    // ============================================================
    ...
    public static function create_appointment($request) {
        ...
        // Pre-check for existing appointment at this slot (more reliable than catching DB error)
        $existing = $wpdb->get_var($wpdb->prepare(
            "SELECT id FROM {$wpdb->prefix}snab_appointments
        ...
        if ($existing) {
            $wpdb->query('ROLLBACK');
            SNAB_Logger::warning('Slot already booked (pre-check)', array(
    ...
    public static function get_user_appointments($request) {
        ...
        MLD_JWT_Handler::send_no_cache_headers();
    ...
    public static function get_appointment($request) {
        ...
        // Allow access if user booked the appointment OR is the assigned staff
        if ($staff_id) {
    ...
    public static function cancel_appointment($request) {
        ...
        // Get appointment - allow access if user booked OR is assigned staff
        if ($staff_id) {
            $appt = $wpdb->get_row($wpdb->prepare(
        ...
        // Only require reason from clients, not staff
        if (!$is_staff_cancelling && $require_reason && empty($reason)) {
            return new WP_REST_Response(array(
                'success' => false,
    ...
    public static function reschedule_appointment($request) {
        ...
        // Determine if user is staff (staff can reschedule without time restrictions)
        $is_staff_rescheduling = $staff_id && ($appt->staff_id == $staff_id);
        ...
        // Staff can reschedule anytime; clients must respect the reschedule deadline
        if (!$is_staff_rescheduling && $hours_until < $reschedule_hours) {
            return new WP_REST_Response(array(
                'success' => false,
        ...
        // Staff can reschedule unlimited times; clients have a limit
        if (!$is_staff_rescheduling && $appt->reschedule_count >= $max_reschedules) {
            return new WP_REST_Response(array(
                'success' => false,
    ...
    public static function register_device_token($request) {
        ...
        // iOS APIClient.request<T>() expects all responses to have a 'data' field
        return new WP_REST_Response(array(
            'success' => true,
            'message' => 'Device token registered successfully',
            'data' => array('id' => is_numeric($result) ? (int) $result : null)
        ), 200);
    }
