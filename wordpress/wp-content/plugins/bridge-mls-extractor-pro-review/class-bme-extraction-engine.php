...
class BME_Extraction_Engine {
    ...
    private function check_extraction_health($extraction_id) {
        try {
            ...
            $running_extraction = get_transient('bme_extraction_running_' . $extraction_id);
            if ($running_extraction) {
                $this->log_extraction_step($extraction_id, 'warning', 
                    "Extraction already running, preventing duplicate session"
                );
                return false;
            }

            // Set running flag
            set_transient('bme_extraction_running_' . $extraction_id, time(), 7200); // 2 hours
    ...
    private function log_extraction_completion($extraction_id, $metrics) {
        ...
        update_post_meta($extraction_id, '_bme_last_run_time', current_time('timestamp'));
