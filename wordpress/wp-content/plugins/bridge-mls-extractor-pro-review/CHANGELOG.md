...
## Version 4.0.37 (January 20, 2026) - 🔧 Fix Duplicate Listings in BOTH Tables

### 🚨 CRITICAL FIX

#### **Duplicate Listings When Record Exists in Both Tables**
- **Root Cause**: v4.0.36 fix used `move_listing_to_active()` which tried to INSERT into the target table. If the listing already existed in the target table (unique key on listing_key), the INSERT failed silently and the record in the wrong table was never deleted.

- **Scenario**: Listing 73454722 (99 Grove St) existed in BOTH `wp_bme_listings` (Active) AND `wp_bme_listings_archive` (Active Under Contract). The v4.0.36 move function couldn't insert a duplicate, so both records remained.

- **Fix**: Added check for listing existence in BOTH tables. If found in both, simply delete from the wrong table and continue with normal processing (which updates the existing record in the correct table).

### Technical Changes

#### `/includes/class-bme-data-processor.php`
- **Lines 416-480**: Enhanced cross-table check to detect when listing exists in BOTH tables
- **New logic**: If in both tables → delete from wrong table, then continue normal processing
- **If only in opposite table** → use move functions as before
- **Cleanup**: Also deletes related data (details, location, financial, features) from wrong table

---

## Version 4.0.36 (January 19, 2026) - 🔧 Fix Duplicate Listings in Summary Tables

### 🚨 CRITICAL FIX

#### **Duplicate Listings Bug (91,346 affected)**
- **Root Cause**: When a listing's status changed (e.g., Active → Closed), `process_single_listing()` routed it to the archive table, but `process_core_listing()` only checked the target table for existing records. The old record in the source table was never deleted, causing duplicates in both `wp_bme_listing_summary` and `wp_bme_listing_summary_archive`.

- **Fix 1 - Cross-table check**: Added logic in `process_single_listing()` to check if the listing exists in the OPPOSITE table before processing. If found, uses proper `move_listing_to_archive()` or `move_listing_to_active()` functions that handle all related tables correctly.

- **Fix 2 - Defensive delete**: Added defensive delete in `process_listing_summary()` that removes any existing record from the opposite summary table before writing.

### Technical Changes

#### `/includes/class-bme-data-processor.php`
- **Lines 416-447**: Added cross-table existence check before processing listings
- **Lines 790-793**: Added defensive delete from opposite summary table
- **Error logging**: Added v4.0.36 log markers for debugging moved listings
...
### Cleanup Required (one-time after deployment)
```sql
DELETE s FROM wp_bme_listing_summary s
INNER JOIN wp_bme_listing_summary_archive a ON s.listing_id = a.listing_id
WHERE s.standard_status = 'Closed' AND a.standard_status = 'Closed';
```
