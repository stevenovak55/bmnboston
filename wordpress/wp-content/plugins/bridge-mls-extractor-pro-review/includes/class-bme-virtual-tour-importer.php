...
class BME_Virtual_Tour_Importer {
    ...
    private function update_virtual_tour_entry($mls_id, $new_tour_urls) {
        ...
        // Convert MLS ID to listing_id (INT)
        $listing_id = intval($mls_id);
        
        $existing_entry = $wpdb->get_row(
            $wpdb->prepare("SELECT * FROM {$this->virtual_tours_table} WHERE listing_id = %d", $listing_id),
            ARRAY_A
        );
        ...
        if ($existing_entry) {
            ...
            if (!empty($update_data)) {
                $wpdb->update($this->virtual_tours_table, $update_data, ['listing_id' => $listing_id]);
