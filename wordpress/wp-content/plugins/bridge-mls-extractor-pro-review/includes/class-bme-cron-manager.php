 ...
 * Version: 2.1.3 (Added 2-minute batch fallback check for large extractions)
 *
 * Changes in 2.1.3:
 * - Added dedicated 2-minute batch fallback check cron
 * - Improved reliability for large extractions (6000+ listings)
 */
class BME_Cron_Manager {
    ...
    const BATCH_FALLBACK_HOOK = 'bme_batch_fallback_check';
    ...
    private function init_hooks() {
        ...
        // New in 2.1.3: Schedule batch fallback check every 2 minutes
        // This ensures stalled batch extractions are resumed quickly
        add_action(self::BATCH_FALLBACK_HOOK, [$this, 'check_batch_fallbacks']);
        if (!wp_next_scheduled(self::BATCH_FALLBACK_HOOK)) {
            wp_schedule_event(time(), 'every_2_minutes', self::BATCH_FALLBACK_HOOK);
        }
    ...
    public function add_cron_schedules($schedules) {
        // New: 2-minute schedule for batch fallback checks
        $schedules['every_2_minutes'] = [
            'interval' => 120, // 2 minutes
            'display'  => __('Every 2 Minutes', 'bridge-mls-extractor-pro')
        ];
    ...
    public function run_scheduled_extractions() {
        ...
        try {
            ...
            foreach ($extractions as $extraction) {
                ...
                // Check if it's time to run (use UTC time() for consistency)
                if (time() >= $next_run) {
                    // Check if extraction is already running to prevent concurrent executions
                    $running_lock = get_transient("bme_extraction_running_{$extraction_id}");

                    if ($running_lock) {
                        $skipped++;
                        $this->log_cron_activity("Skipped extraction (already running): {$extraction->post_title}");
                        continue;
                    }

                    try {
                        // Set running lock for 2 hours
                        set_transient("bme_extraction_running_{$extraction_id}", time(), 2 * HOUR_IN_SECONDS);

                        // Update last_run_time at START to ensure true 15-min intervals
                        // Store in UTC (time()) - display code uses wp_date() for local time
                        ...
                        // Always clear the running lock when extraction finishes
            ...
            // Check for batch fallback triggers
            try {
                $fallback_executed = BME_Batch_Manager::check_fallback_triggers();
                if ($fallback_executed > 0) {
                    $this->log_cron_activity("Executed {$fallback_executed} batch fallback triggers");
                }
            } catch (Exception $e) {
                $this->log_cron_activity('Error checking batch fallbacks: ' . $e->getMessage());
            }
    ...
    public function run_cleanup_tasks() {
        ...
        try {
            ...
            // Run sync health check and auto-fixes (temporarily disabled)
            /*try {
                $health_checker = bme_pro()->get('sync_health_checker');
                $health_results = $health_checker->run_and_save_health_check();

                if ($health_results['auto_fixes_applied'] > 0) {
                    $this->log_cron_activity("Health check applied {$health_results['auto_fixes_applied']} auto-fixes");
                }
            } catch (Exception $e) {
                $this->log_cron_activity("Health check error: " . $e->getMessage());
            }*/
    ...
    public function run_virtual_tour_import() {
        ...
        update_option('bme_pro_last_vt_import_time', current_time('timestamp'));
        update_option('bme_pro_last_vt_import_status', $success ? 'Success' : 'Failed');
        update_option('bme_pro_last_vt_import_duration', $duration);
    }

    /**
     * Check and execute batch fallback triggers
     *
     * This runs every 2 minutes to ensure stalled batch extractions are resumed.
     * Much more frequent than the 15-minute master cron check to prevent
     * extractions from appearing "stuck" for long periods.
     *
     * @since 2.1.3
     */
    public function check_batch_fallbacks() {
        try {
            $fallback_executed = BME_Batch_Manager::check_fallback_triggers();
            if ($fallback_executed > 0) {
                $this->log_cron_activity("Batch fallback check: Executed {$fallback_executed} pending triggers");
            }
        } catch (Exception $e) {
            $this->log_cron_activity('Batch fallback check error: ' . $e->getMessage());
        }
    }
    ...
    public function maybe_reschedule_cron() {
        ...
        if ((current_time('timestamp') - $last_check) < 3600) {
            return;
        }

        update_option('bme_pro_last_cron_check', current_time('timestamp'));
        ...
        // Check batch fallback cron (new in 2.1.3)
        $next_batch_fallback = wp_next_scheduled(self::BATCH_FALLBACK_HOOK);
        if (!$next_batch_fallback) {
            wp_schedule_event(time(), 'every_2_minutes', self::BATCH_FALLBACK_HOOK);
            $this->log_cron_activity('Rescheduled missing batch fallback cron');
        }
    }
    ...
    public function get_cron_status() {
        $status = [
            ...
            'batch_fallback_cron' => [
                'scheduled' => wp_next_scheduled(self::BATCH_FALLBACK_HOOK),
                'next_run' => wp_next_scheduled(self::BATCH_FALLBACK_HOOK) ?
                    'In ' . human_time_diff(wp_next_scheduled(self::BATCH_FALLBACK_HOOK)) : 'Not scheduled',
                'interval' => '2 minutes',
                'purpose' => 'Ensures large batch extractions continue without stalling',
            ],
            'wordpress_cron' => [
        ...
        foreach ($extractions as $extraction) {
            ...
            if (isset($schedules[$schedule])) {
                ...
                $scheduled[] = [
                    ...
                    'last_run' => $last_run ? human_time_diff($last_run, current_time('timestamp')) . ' ago' : 'Never',
                    'next_run' => $next_run > current_time('timestamp') ?
                        'In ' . human_time_diff($next_run, current_time('timestamp')) :
                        'Due now',
                    'is_due' => $next_run <= current_time('timestamp')
                ];
            }
        }
    ...
    public function clear_all_schedules() {
        ...
        wp_clear_scheduled_hook(self::BATCH_FALLBACK_HOOK);
    ...
    private function update_cron_stats($executed, $failed, $skipped, $duration) {
        ...
        $stats['last_run'] = current_time('timestamp');
