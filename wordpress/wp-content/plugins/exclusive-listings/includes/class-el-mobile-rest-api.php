...
class EL_Mobile_REST_API {
    ...
    public function handle_update($request) {
        ...
        $new_status = $sanitized['standard_status'] ?? $old_status;
    ...
    public function handle_delete($request) {
        $listing_id = intval($request->get_param('id'));
        $archive = $request->get_param('archive') !== 'false';
        ...
        if (isset($files['photos'])) {
            ...
            if (is_array($photos['name'])) {
                for ($i = 0; $i < count($photos['name']); $i++) {
                    ...
                    $results[] = $this->process_photo_upload($listing_id, $file, $i);
                }
            } else {
                $results[] = $this->process_photo_upload($listing_id, $photos, 0);
            }
        } elseif (isset($files['photo'])) {
            $results[] = $this->process_photo_upload($listing_id, $files['photo'], 0);
        }
    ...
    /**
     * Process a single photo upload and format the result
     *
     * @since 1.5.2
     * @param int $listing_id Listing ID
     * @param array $file File data
     * @param int $index Photo index
     * @return array Upload result
     */
    private function process_photo_upload($listing_id, $file, $index) {
        ...
        return array(
            'filename' => $file['name'],
            'success' => !$is_error,
            'data' => $is_error ? null : $this->format_photo_for_ios($result, $index),
    ...
    public function handle_reorder_photos($request) {
        $listing_id = intval($request->get_param('id'));
        $order = $request->get_json_params()['order'] ?? null;

        if (empty($order) || !is_array($order)) {
            return $this->error_response('invalid_order', 'Order must be an array of photo IDs', null, 400);
        }

        $result = $this->image_handler->reorder_photos($listing_id, $order);
        if (is_wp_error($result)) {
            return $this->error_response($result->get_error_code(), $result->get_error_message(), null, 500);
        }
    ...
    private function get_agent_id($user_id) {
        ...
        $table_exists = $wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $agent_table));
        if (!$table_exists) {
            return null;
        }
    ...
    private function format_listing_response($listing) {
        ...
        return array(
            ...
            'architectural_style' => $details['architectural_style'] ?? null,
            'stories_total' => isset($details['stories_total']) ? intval($details['stories_total']) : null,
            'virtual_tour_url' => $details['virtual_tour_url_unbranded'] ?? null,
            'public_remarks' => $details['public_remarks'] ?? null,
            'private_remarks' => $details['private_remarks'] ?? null,
            'showing_instructions' => $details['showing_instructions'] ?? null,

            // Tier 2 - Interior Details
            'heating' => $details['heating'] ?? null,
            'cooling' => $details['cooling'] ?? null,
            'heating_yn' => !empty($details['heating_yn']),
            'cooling_yn' => !empty($details['cooling_yn']),
            'interior_features' => $details['interior_features'] ?? null,
            'appliances' => $details['appliances'] ?? null,
            'flooring' => $details['flooring'] ?? null,
            'laundry_features' => $details['laundry_features'] ?? null,
            'basement' => $details['basement'] ?? null,

            // Tier 3 - Exterior & Lot
            'construction_materials' => $details['construction_materials'] ?? null,
            'roof' => $details['roof'] ?? null,
            'foundation_details' => $details['foundation_details'] ?? null,
            'exterior_features' => $features['exterior_features'] ?? null,
            'waterfront_yn' => !empty($features['waterfront_yn']),
            'waterfront_features' => $features['waterfront_features'] ?? null,
            'view_yn' => !empty($features['view_yn']),
            'view' => $features['view'] ?? null,
            'parking_features' => $details['parking_features'] ?? null,
            ...
            'association_yn' => !empty($financial['association_yn']),
            'association_fee' => isset($financial['association_fee']) ? floatval($financial['association_fee']) : null,
            'association_fee_frequency' => $financial['association_fee_frequency'] ?? null,
            'association_fee_includes' => $financial['association_fee_includes'] ?? null,

            // Features (legacy boolean flags)
            'has_pool' => !empty($listing['has_pool']),
            'has_fireplace' => !empty($listing['has_fireplace']),
            'has_basement' => !empty($listing['has_basement']),
            'has_hoa' => !empty($listing['has_hoa']),
    ...
    private function format_photos_array_for_ios($photos) {
        ...
        foreach ($photos as $index => $photo) {
            $formatted[] = array(
                'id' => intval($photo['id']),
                'url' => $photo['media_url'],
                'sort_order' => intval($photo['order_index'] ?? ($index + 1)),
                'is_primary' => $index === 0,
            );
        }
        return $formatted;
