...
class EL_Admin {
    ...
    private function handle_update() {
        ...
        $new_status = $data['standard_status'] ?? $old_status;
    ...
    private function handle_delete() {
        $listing_id = intval($_POST['listing_id']);
        $should_archive = isset($_POST['archive']) && $_POST['archive'] === '1';
        ...
        $action = $should_archive ? 'archived' : 'deleted';
        $this->redirect_with_message('exclusive-listings', 'success', 'Listing ' . $action . ' successfully');
    }
    ...
    public function render_list_page() {
        ...
        if (isset($_GET['action'], $_GET['id']) && $_GET['action'] === 'delete') {
            if (wp_verify_nonce($_GET['_wpnonce'], 'el_delete_' . $_GET['id'])) {
                $this->bme_sync->archive_listing(intval($_GET['id']));
    ...
    public function render_add_page() {
        ...
        if (isset($_GET['message'])) {
            ...
            $message = $message_map[$_GET['message']] ?? 'Operation completed.';
    ...
    private function sync_photos_to_bme($listing_id) {
        ...
        foreach ($attachments as $attachment) {
            ...
            $wpdb->insert($media_table, array(
                ...
                'media_key' => md5($listing_id . '_' . $url . '_' . current_time('timestamp') . '_' . $order),
                'media_url' => $url,
        ...
        if ($order > 0) {
            $wpdb->update(
                $summary_table,
                array(
                    'main_photo_url' => wp_get_attachment_url($attachments[0]->ID),
                    'photo_count' => $order,
                ),
