...
class BMN_Schools_Ranking_Calculator {
     ...
     */
    private $weights = [
        'mcas_proficiency' => 0.40,   // 40% - MCAS proficiency scores
        'graduation_rate'  => 0.12,   // 12% - 4-year graduation rate
        'mcas_growth'      => 0.10,   // 10% - Year-over-year improvement
        'ap_performance'   => 0.09,   // 9%  - AP participation/pass rate (increased from 6%)
        'masscore'         => 0.08,   // 8%  - MassCore completion %
        'attendance'       => 0.08,   // 8%  - Inverse of chronic absence
        'ratio'            => 0.05,   // 5%  - Student-teacher ratio (reduced from 8%)
        'per_pupil'        => 0.04,   // 4%  - Per-pupil spending index
        'college_outcomes' => 0.04,   // 4%  - College enrollment rate (district-level)
    ];
    ...
    private $weights_elementary = [
        'mcas_proficiency' => 0.45,   // 45% - MCAS proficiency (primary metric)
        'attendance'       => 0.20,   // 20% - Inverse of chronic absence
        'mcas_growth'      => 0.15,   // 15% - Year-over-year improvement
        'per_pupil'        => 0.12,   // 12% - Per-pupil spending (increased from 10%)
        'ratio'            => 0.08,   // 8%  - Student-teacher ratio (reduced from 10%)
        'graduation_rate'  => 0.00,   // N/A for elementary
        'masscore'         => 0.00,   // N/A for elementary
        'ap_performance'   => 0.00,   // N/A for elementary
        'college_outcomes' => 0.00,   // N/A for elementary
    ];
    ...
    public function __construct($year = null) {
        $this->year = $year ?: self::get_latest_data_year();
    }

    /**
     * Lock key for preventing concurrent ranking calculations.
     *
     * @since 0.6.36
     */
    const LOCK_KEY = 'bmn_schools_ranking_lock';

    /**
     * Get the latest year that has ranking data.
     *
     * This prevents year rollover bugs where date('Y') returns the new year
     * but ranking data is from the previous year. For example, on Jan 1, 2027,
     * date('Y') returns 2027 but rankings are from 2026.
     *
     * @since 0.6.37
     * @return int The latest year with ranking data, or current year - 1 as fallback.
     */
    public static function get_latest_data_year() {
        global $wpdb;

        $rankings_table = $wpdb->prefix . 'bmn_school_rankings';

        $latest_year = $wpdb->get_var("SELECT MAX(year) FROM {$rankings_table}");

        if ($latest_year) {
            return (int) $latest_year;
        }

        // Fallback: assume data is from previous year
        return (int) date('Y') - 1;
    }

    /**
     * Lock duration in seconds (10 minutes max).
     *
     * @since 0.6.36
     */
     ...
     *
     * Uses a database lock to prevent race conditions when multiple processes
     * try to calculate rankings simultaneously. Also wraps database writes in
     * a transaction for data integrity.
     *
     * @since 0.6.0
     * @since 0.6.36 Added lock to prevent race conditions and database transaction
    ...
    public function calculate_all_rankings() {
        ...
        if (!$this->acquire_lock()) {
            if (class_exists('BMN_Schools_Logger')) {
                BMN_Schools_Logger::log('warning', 'ranking', 'Ranking calculation already in progress, skipping');
            }
            return [
                'error' => 'Ranking calculation already in progress',
                'locked' => true,
            ];
        }
        ...
        try {
            ...
            // Store rankings in database with transaction
            ...
            try {
                ...
                if (class_exists('BMN_Schools_Logger')) {
                    BMN_Schools_Logger::log('error', 'ranking', 'Failed to store rankings', [
                        'error' => $e->getMessage()
                    ]);
                }
    ...
    /**
     * Acquire a lock to prevent concurrent ranking calculations.
     *
     * @since 0.6.36
    ...
    /**
     * Release the ranking calculation lock.
     *
     * @since 0.6.36
     */
    ...
    public function calculate_school_score($school_id, $level = null) {
        ...
        $components = [
            ...
            'college_outcomes' => $this->get_college_outcomes_score($school_id),
        ];
    ...
    private function get_ratio_score($school_id) {
        ...
        // Get all staffing records and find the one with the highest year
        ...
        // Find the staffing record with the highest year
        $latest_staffing = null;
        $latest_year = 0;
        foreach ($results as $row) {
            $value = json_decode($row->feature_value, true);
            $year = isset($value['year']) ? (int) $value['year'] : 0;
            if ($year > $latest_year) {
                $latest_year = $year;
                $latest_staffing = $value;
            }
        }

        if ($latest_staffing !== null) {
            if (isset($latest_staffing['student_teacher_ratio'])) {
                $ratio = (float) $latest_staffing['student_teacher_ratio'];
            } elseif (isset($latest_staffing['teacher_fte']) && $latest_staffing['teacher_fte'] > 0) {
                // Calculate from demographics - use matching year if available
                $demo_table = $wpdb->prefix . 'bmn_school_demographics';
                $enrollment = $wpdb->get_var($wpdb->prepare(
                    "SELECT total_students FROM {$demo_table} WHERE school_id = %d AND year = %d LIMIT 1",
                    $school_id, $latest_year
                ));
                // Fall back to most recent year if no match
                if (!$enrollment) {
                    $enrollment = $wpdb->get_var($wpdb->prepare(
                ...
                if ($enrollment) {
                    $ratio = (float) $enrollment / (float) $latest_staffing['teacher_fte'];
                }
            }
        }
    ...
    /**
     * Get college outcomes score for a school (0-100).
     * Uses district-level data since college outcomes are tracked at district level.
     *
     * @param int $school_id School ID.
     * @return float|null Score or null if no data.
     */
    private function get_college_outcomes_score($school_id) {
        global $wpdb;
        ...
        $district_data = $wpdb->get_row($wpdb->prepare(
            "SELECT d.extra_data
             FROM {$schools_table} s
             JOIN {$districts_table} d ON s.district_id = d.id
             WHERE s.id = %d",
        ...
        if (!$district_data || empty($district_data->extra_data)) {
            return null;
        }

        $extra = json_decode($district_data->extra_data, true);
        if (!isset($extra['college_outcomes']) || !isset($extra['college_outcomes']['total_postsecondary_pct'])) {
            return null;
        }

        $college_pct = (float) $extra['college_outcomes']['total_postsecondary_pct'];

        // Score based on college enrollment percentage:
        // MA average is around 65%. Higher = better.
        // 40% = 40, 65% = 65, 85%+ = 100
        // Essentially the percentage IS the score, capped at 100
        return min(100, $college_pct);
    }
    ...
    public static function get_letter_grade($score) {
        trigger_error('get_letter_grade() is deprecated. Use get_letter_grade_from_percentile() for fairer percentile-based grading.', E_USER_DEPRECATED);
    ...
    public static function get_school_ranking($school_id, $year = null) {
        ...
        $year = $year ?: self::get_latest_data_year();
        $table = $wpdb->prefix . 'bmn_school_rankings';
